...
limiter = Limiter(key_func=get_remote_address, default_limits=["5/minute"])
parqet_router.limiter = limiter
# Middleware moved to main app(SlowAPIMiddleware)
...
@parqet_router.post("/proxy")
@parqet_router.get("/proxy")
...
from fastapi import APIRouter
tempest_router = APIRouter()

logger = logging.getLogger("uvicorn")

# ✅ Initialize Rate Limiter (5 requests per minute per IP)
limiter = Limiter(key_func=get_remote_address, default_limits=["5/minute"])
tempest_router.limiter = limiter
# Middleware moved to main app(SlowAPIMiddleware)
...
@tempest_router.post("/proxy")
@tempest_router.get("/proxy")
@limiter.limit("5/minute")  # ⏳ Apply rate limit (5 requests per minute per IP)
async def proxy_request(request: Request):
    """Secure JSON proxy with rate limiting."""
    logger.info(
...
app.include_router(parqet_router, prefix="/parqet")
app.include_router(tempest_router, prefix="/tempest")

@app.get("/")
async def root():
    return {"message": "Combined Proxy Server Running"}
